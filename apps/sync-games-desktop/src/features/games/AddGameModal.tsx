...
export function AddGameModal({ isOpen, onClose, onSuccess, initialPath = "", suggestedId = "" }: AddGameModalProps) {
  ...
  const handleOpenChange = (isOpenModal: boolean) => {
    if (!isOpenModal) {
      setError(null);
      setGameId("");
      setPath("");
  ...
  const handleSubmit = async () => {
    ...
    setError(null);
    try {
      await addGame(id, p, edition || undefined, url || undefined, selectedSteamAppId || undefined);
      onSuccess();
