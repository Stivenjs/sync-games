...
export function useTorrent() {
  const startTorrentDownloadMutation = useMutation({
    mutationFn: ({ magnet, savePath }: { magnet: string; savePath: string }) =>
      startTorrentDownload(magnet, savePath),
  });
  const startTorrentFileDownloadMutation = useMutation({
    mutationFn: ({ file, savePath }: { file: string; savePath: string }) =>
      startTorrentFileDownload(file, savePath),
  });
