...
function collectFilesRecursive(
  ...
  for (const e of entries) {
    ...
    } else if (e.isFile() && looksLikeSaveFile(e.name)) {
      out.push({
        absolute: full,
...
/**
 * Lista archivos que parecen guardados en las rutas dadas.
 * Cada path puede ser archivo o carpeta; se expanden variables de entorno.
 * Devuelve { absolutePath, relativePath } para cada archivo (relative respecto al path que lo contiene).
 */
export function listSaveFilesFromPaths(
  ...
  for (const raw of paths) {
    ...
    if (stat.isFile()) {
      if (looksLikeSaveFile(expanded)) {
        results.push({ absolute: expanded, relative: basename(expanded) });
      }
    } else if (stat.isDirectory()) {
      collectFilesRecursive(expanded, expanded, results);
    }
  }
